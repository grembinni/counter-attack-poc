...
- [ ] **RULE-01**: On a High Pass, the header contestant selection phase is triggered only after the accuracy check roll resolves — not before the roll
- [ ] **RULE-02**: On a High Pass with header contestants, the target hex selection step is triggered only after the header contestant duel resolves — correct sequence is: accuracy check → contestant duel → target selection
