...
- [ ] **HILITE-03**: The "selected" piece ring and the "already-moved-this-stage" piece ring are visually distinct from each other (currently both identical green, differing only by ring radius)
...
- [ ] **CLEANUP-01**: Dead code (unused fields, functions, exports) is identified and removed across shared/server/client, verified via an automated dead-code-detection tool
- [ ] **CLEANUP-02**: Duplicated logic (e.g. repeated `TEAM_CONFIGS` color lookups, repeated team-slot derivation) is consolidated into shared helper functions/hooks
- [ ] **CLEANUP-03**: Zustand store selectors are reviewed for staleness and inefficiency (e.g. stale dependency arrays, redundant derived-state computation)
