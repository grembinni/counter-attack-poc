# Requirements Archive — Counter Attack Web v1.3
...
> **Archive note:** All requirements marked `[x]` were satisfied. BUG-23 was the one genuine gap acknowledged at milestone close — escalated to Phase 26 (v1.4 tech debt). Requirements FORM-01..04, OFFSIDE-01/02, REPLAY-07/08, UX-15 were implemented but their REQUIREMENTS.md checkboxes were stale at close; final status verified from PROJECT.md and phase SUMMARY.md evidence.
...
- [x] **PALETTE-01**: Each team's color definition has exactly 4 values: `primary` (main team color), `primaryLight` (primary color lightened 3 shades), `secondary1`, `secondary2` _(D-08: evolved to 6-field home/away model in implementation; intent satisfied)_
...
- [x] **UNIFORM-01**: A uniform style library is defined in `packages/shared`; each style is a named SVG pattern template parameterized by a `TeamPalette`; each style defines both an outfield rendering and a GK variant
...
- [x] **FORM-01**: A formation selection screen appears after uniform selection and before KICK_OFF_SETUP; each player independently chooses one of: 4-4-2, 5-3-2, 4-3-3, 3-4-3
- [x] **FORM-02**: Each formation option displays a mini pitch diagram showing approximate piece positions and a one-line tactical description
- [x] **FORM-03**: Both players must confirm their formation choice before the game advances; each player sees "waiting for opponent" after confirming
- [x] **FORM-04**: Piece starting positions are placed dynamically from a `FORMATIONS` definition table keyed by `FormationId`; away positions are the symmetric mirror (q = 36 − home_q)
...
- [x] **OFFSIDE-01**: Two-tab live UAT closure — offside detection formally closed (Phase 25; all three scenarios + stickiness/clear confirmed)
- [x] **OFFSIDE-02**: Two-tab live UAT closure — free-kick restart formally closed (Phase 25; FREE_KICK_SETUP 6-step sequence redesign completed and UAT-passed in Plan 25-06)
- [x] **REPLAY-07**: GK_KICK ball delivery visible during post-game replay — `ballAfter` field added to GK_KICK ActionEvent; REPLAY_ELIGIBLE_TYPES updated (Phase 25 Plan 01)
- [x] **REPLAY-08**: LOOSE_BALL_LAND ball delivery visible during post-game replay — same treatment as REPLAY-07 (Phase 25 Plan 01)
- [x] **BUG-22**: HIGH_PASS_MOVE excludes `highPassCarrierId` from repositioning target — fix confirmed shipped in Phase 18.2; closed in Phase 25 (D-04/D-05)
- [ ] **BUG-23**: KICK_OFF_SETUP shot-path hex shading clears correctly after a SNAPSHOT_DEFLECT goal ⚠️ **KNOWN GAP — not resolved at v1.3 close; escalated to Phase 26 (v1.4 tech debt)**
- [x] **UX-15**: UX streamlining — jersey number centering (Plan 25-08), Style 12 pattern origin fix (Plan 25-09), EventBanner pass-result popup (Plan 25-03), uniform-selection race fix (Plan 25-04) implemented and UAT-closed
...
| OFFSIDE-01 | 25    | ✅ Complete | UAT closed Phase 25 two-tab session                                                         |
| OFFSIDE-02 | 25    | ✅ Complete | UAT closed after Plan 25-06 FREE_KICK_SETUP redesign                                        |
| REPLAY-07  | 25    | ✅ Complete |                                                                                             |
| REPLAY-08  | 25    | ✅ Complete |                                                                                             |
| BUG-22     | 25    | ✅ Complete | Fix shipped Phase 18.2: `carrierExclusionKey: 'highPassCarrierId'` at `gameHandlers.ts:405` |
| BUG-23     | 25    | ⚠️ Gap      | Root cause unresolved; escalated to Phase 26 (v1.4 tech debt)                               |
| UX-15      | 25    | ✅ Complete | Jersey centering, Style 12, EventBanner, uniform-selection race fix                         |

**Final count: 40/41 requirements satisfied. 1 known gap (BUG-23) acknowledged at milestone close.**

---

_Archive created 2026-07-12. For current project status, see .planning/ROADMAP.md_
