...
## Milestone Summary — v1.4 Response Polish + Draft Mode

**Status:** SHIPPED 2026-07-22 with one known gap (see below) — see [.planning/v1.4-MILESTONE-AUDIT.md](v1.4-MILESTONE-AUDIT.md) for full evidence.

**Decimal/Inserted Phases:** None — Phases 26-30 are all whole-numbered; Phase 29 absorbed 6 gap-closure sub-plans (29-07..29-12) rather than being split into inserted phases.

**Key Decisions:**

- Draft mode built as a pure, socket-independent state machine (`draftEngine.ts` / `draftSession.ts`) mirroring `gameEngine.ts`'s separation from the socket layer — kept draft logic server-authoritative and independently testable (Phase 28/29).
- Phase 30 intentionally redesigned and superseded Phase 28/29's 5-tier/7-card/keeper-safety-net draft model with a 4-tier/6-round/dedicated-GK-round model — a mid-milestone pivot based on live-playtest feedback, not a defect fix. All three superseded requirements (DRAFT-04, DRAFT-05, DRAFT-08) are documented as "superseded design" in the requirements archive rather than silently overwritten.
- Draft carousel and lineup/bench screens intentionally merged into one interaction (drag card → lineup slot or bench directly) rather than the roadmap's originally-described separate "pick screen" then "lineup screen" (D-05, Phase 29) — a deliberate scope interpretation made during execution.

**Issues Resolved:**

- 6 known gameplay bugs (undo scoping, End Turn button color, opponent stats click, deflection log format, header duel target hex, shot range validation) — Phase 26.
- 5 gaps surfaced by Phase 29's first two-browser human verification (post-draft rearrangement, bench carousel, game-start hand-off) — closed across gap-closure plans 29-07 through 29-12, each independently re-verified.

**Issues Deferred:**

- **RESP-01 through RESP-09** (Response Activation model — 9 requirements, half of this milestone's namesake goal): never implemented. The original roadmap had a Phase 27 titled "Response Activation Model"; it was replaced by "Game Creation Settings" and RESP-01..09 were never picked up by any later phase. Deferred in full to a future milestone.
- **DRAFT-10 human verification**: the live two-browser lineup-slot-swap walkthrough required by Phase 29's own plan was never actually performed (protocol-layer tests pass; Phase 29's final status is `human_needed`, not `passed`).
- **createServer.ts reconnect bug** (misplaced `return` at line ~166, pre-existing since Phase 07): a reconnecting socket whose room isn't currently `'playing'` never gets handlers registered. Flagged by Phase 29's own verification as out-of-scope, and again by this milestone's integration check — still unfixed.
- Phase 28 has no Nyquist `VALIDATION.md` — compliance was never scored for that phase (missing, not failing).

**Technical Debt Incurred:**

- Several stale code comments left behind by the Phase 28→30 redesign (`roomHandlers.ts` "8 packs" comment now describes a 12-pack/6-round reality; `GameSettingsScreen.tsx`/`types.ts` comments still describe Legends/Icons as non-selectable after Phase 30 enabled them).
- `DRAFT_REARRANGE` bench-index bounds validation gap (not currently exploitable).
- Bench jersey numbers not assigned for cards benched via a post-draft-complete `DRAFT_REARRANGE` (cosmetic).
- Client-side tier-color cache does not survive reconnect/reload (cosmetic).
- REQUIREMENTS.md traceability staleness for DRAFT-01/02/03 (said "Phase 28") and DRAFT-06 (said "Phase 30", unchecked) — corrected in the final archive above, but the live document carried the stale rows through most of the milestone.
