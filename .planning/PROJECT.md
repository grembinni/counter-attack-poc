...
**Phase 47 (Select-Based Roster Interaction) complete 2026-08-31.** Validated in Phase 47: ROSTER-01 through ROSTER-08. Delivered: every roster/lineup surface (mid-match positioning, mid-match substitution, Standard pregame slot swap, draft-mode pack/bench/lineup) ported from native HTML5 drag-and-drop to the click-to-select model (green selected / blue eligible target) used everywhere else in the game, across 6 plans/5 waves — establishing the shared `.statCardSelected`/`.statCardEligible` selection tokens and `DraftCardBody` click contract first (Wave 1), then `BenchCarousel` (Wave 2), then the `LineupAssignmentScreen.tsx` production port itself (Wave 3, landed atomically per its own "a partially ported file does not compile" constraint), then the ~90-case `LineupAssignmentScreen.test.tsx` rewrite plus `GameBoard.test.tsx`/`CardInjuryBadge.crossSurface.test.tsx` collateral updates in parallel (Wave 4), closing with a `knip`-clean, structurally-separate (6 eligibility functions + 8 click handlers, none mode-parameterized), fully green phase gate (Wave 5). Zero-regression, first phase of v1.8; no other v1.8 phase touches these files. Test suite: shared 908 / server 1635 (1 skipped, 1 todo) / client 1287 = 3,830 tests, all green. Code review: 0 critical, 4 warnings (non-blocking maintainability notes), 3 info.

**Phase 48 (Permanent Jersey Numbers) complete 2026-08-31.** Validated in Phase 48: NUMBER-01 through NUMBER-05 (5/5 must-haves). Delivered: jersey numbers as a stable, permanent property of the person rather than the formation/lineup slot, across 6 plans/3 waves — `applyRosterReposition` stopped re-binding `number` to the slot on swap (Wave 1, 48-01), bench numbers unified onto one server-authoritative crypto-backed 15-99 draw for both standard and draft rooms via a new `backfillBenchNumbers` "fill gaps, never re-roll" helper (48-02), and the standard-mode pregame bench stopped fabricating a client-side number that would later change (48-03); `applySubstitution` made both the incoming and outgoing player's numbers person-owned, fixing two live violations the original research had traced but the canonical file list hadn't enumerated (Wave 2, 48-04), and the `DRAFT_REARRANGE` post-draftComplete bench-number orphan gap — silently defaulting to jersey `#0` — was closed (48-05); the kickoff-striker anchor was moved off `number === 9` onto a `slotId === 'ST'` formation-slot lookup, the last place a mutable-turned-permanent number was load-bearing for game logic (Wave 3, 48-06). This also resolves the "bench jersey numbers not assigned on post-complete rearrange" item from the v1.4-era known tech debt list (see below). Test suite: shared 908 / server 1648 (1 skipped, 1 todo) / client 1293 = 3,849 tests, all green. Code review: 0 critical, 2 warnings (stale doc comment, inconsistent bench-number fail-safe between two call sites), 1 info (duplicated regex).
...
_Last updated: 2026-08-31 after Phase 48 (Permanent Jersey Numbers) completion_
