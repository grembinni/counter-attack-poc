...
**Phase 40 (Substitutions) complete 2026-08-17** — the third phase of v1.6. 7 plans, 8/8 requirements (SUB-01..07, SETTINGS-04) SATISFIED. Delivered: a server-side substitution rules engine (`applySubstitution`/`applyRosterContinuity` in `gameEngine.ts`) enforcing a 9-guard chain (stoppage-phase gate, whole-match 3-sub cap that never resets at half-time, ownership, red-card-out exclusion, bench existence, already-subbed exclusion, red-card-in exclusion, pool identity, GK-slot parity), with jersey number/pitch slot inheritance and a +1-minute added-time fold-in per substitution; a `GAME_SUBSTITUTION` socket handler validating payload shape and ownership before ever touching the engine; a mid-match roster screen adapted from the pre-match `LineupAssignmentScreen` (bench OUT/RED CARD badges, drag-and-drop bench-to-pitch swap) mounted behind a persistent SUB affordance on `GameBoard`; and a two-client socket integration test (12 cases) proving the full flow end to end. A mid-execution human-verify checkpoint on the final integration-test plan (40-07) surfaced a real bug — the mid-match roster panel grouped substituted-in players by their own position role instead of their fixed formation slot, visually collapsing formations (e.g. a 4-4-2 reading as a 4-3-3 after a sub) — plus four UX gaps (roster panel only viewable during a stoppage, rendered as a small modal instead of full-size, bench narrower than the lineup, no visual cue for when a sub is actionable). All five were fixed in a single gap-closure cycle (commits 558e205/066185e/948f94c/bc800ed/f3958b6): grouping now parses each piece's fixed formation-slot index from its server-assigned id, matching the pregame draft screen's own grouping logic exactly; the panel is now always-open (read-only outside a stoppage), full-size, width-matched to the lineup, and the SUB header turns green when a substitution is currently actionable. Human re-verified and approved on the second pass. Code review found 0 critical/blocker issues (2 non-blocking warnings: a cosmetic error-message wording mismatch on a narrow GK-parity rejection path, and a redundant/inert missing guard on a dragover handler). Test suite: shared 839 / server 1439 (1 skipped, 1 todo) / client 998 (3,276 total), all green; full monorepo build and typecheck clean.

**Phase 38 (Corner Kick) complete 2026-08-09** — the second phase of v1.6. 33 plans, 7/7 requirements (OOB-03, CORNER-01..06) SATISFIED. Delivered: a ball exiting over the defending byline after a defending touch awards a corner and repositions both goalkeepers first; the kicking manager places a corner-taker at a fixed corner-arc hex, then both managers alternate repositioning up to 6 players 2-at-a-time; a second, separate pre-kick window lets each side reposition one more player up to 3 hexes; the corner resolves as a High Pass (unlimited distance inside the penalty area, 15-hex cap elsewhere, forces a header) or Low Pass (no header), on the existing combined-score 8+ accuracy check. Four gap-closure rounds followed initial execution (plans 38-09 through 38-33): round 1 (38-10..38-15) fixed two `applyUndo` BLOCKERs and two WARNING-level correctness defects, then corrected a Low-Pass-corner interceptability rule and replaced a spilled-goalkeeper-save placeholder; round 2 (38-16..38-24) closed four defects surfaced by a live two-browser walkthrough — free/uncapped reposition movement, a mandatory pre-corner clear-out step, a missing restart event banner, and a corner-award ruling correction; round 3 (38-25..38-30) replaced the interactive clear-out with an automatic one, converted the unbounded reposition walk into a free-kick-style bounded-area placement (server and client), and fixed an `ActionLog` runtime crash; round 4 (38-31..38-33) fixed the actual root cause of an Undo regression the round-3 checkpoint found — `GameState.lastDiceRoll` was never cleared on entry to either corner reversible-move window, permanently blocking the client's `if (lastDiceRoll) return false` Undo guard for the whole corner sequence despite `applyUndo`'s own LIFO logic already being correct — and formally deferred a second reported bug (offside rings persisting after a goal resets positions for kick-off) out of phase scope with full written triage evidence, tracked at `.planning/todos/pending/2026-08-09-bug-offside-ring-after-goal.md`. The final live two-browser re-verification (38-33) confirmed all fixes and explicitly reconfirmed the D-GAP-01 Undo-slot-refund ruling under the current bounded single-click placement model. Code review across the phase found 0 critical/blocker issues, with 2 non-blocking warnings flagged as follow-ups (an `ActionLog` event-switch exhaustiveness gap; a client/server occupancy-check mismatch on a piece's own hex during corner reposition). Test suite: shared 702 / server 1034 (1 skipped, 1 todo) / client 776 (2,512 total), all green; full monorepo build and typecheck clean.
...
_Last updated: 2026-08-17 after Phase 40 completion_
