...
**Phase 30 complete (2026-07-22)** — draft system recalibrated end-to-end: player pool rebalanced and reclassified into a 4-tier fixed-absolute-threshold scheme (chase/rare/uncommon/common; keeper tier removed); pack generation rewritten to a 6-round, position/GK-constrained, variable-composition structure (GK-only round 1, tiered rounds 2-6, 17 cards/player); forced-keeper-on-cycle-4 safety net deleted; Legends and Icons draft pools enabled; 4-color tier borders extended to bench and starting-11 lineup slots. All 5 phases of v1.4 (26-30) are now marked complete in ROADMAP.md — `phase.complete` reports `is_last_phase: true` with no next phase defined. Note: the v1.4 milestone goal text also lists a "Response activation cleanup" track (single-selection model for response moves) that has no corresponding phase in the roadmap — worth reconciling before treating v1.4 as ready to ship.
