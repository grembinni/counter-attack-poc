...
**v1.7 milestone content complete — Phase 46 (Final Cleanup) complete 2026-08-30.** Sixth and last of 6 phases (7 plans, 4 waves, 9/9 requirements — CLEANUP-05..13 — satisfied). Closed the milestone-wide consistency audit: the ball-location ring now also renders during `FREE_KICK_SETUP`/`FREE_MOVE_ATTACK`/`FREE_MOVE_DEFENSE`; the Free Move panel heading names its trigger (`Final-Third Movement!`); a mid-move piece now stays selected with its ring after an interrupt prompt resolves (closing a gap flagged back in Phase 43); Match Speed relocated into the Advanced Settings drawer; Free Kick kicker-selection copy now matches the Penalty/Corner/Throw-In family; standard (non-Draft) rooms gained a 5-player generic placeholder bench per side; valid-move hex tint consistency was formalized into an explicit, documented, exported exception set; and the pitch player card's badge size was aligned to the roster/bench cards. Plan 46-07 closed the three inherently-manual verifications (cross-restart kicker/thrower feel, card-layout alignment, dead-ball highlight consistency) plus three more live-only checks (Match Speed, standard bench, auto-reselect) via a live two-browser developer walkthrough — all 8 checks passed, recorded verbatim in `46-AUDIT.md`'s Live Verification Record and as an Outcome column on `46-VALIDATION.md`'s Manual-Only table. Verified independently: verifier re-derived evidence from live source (not SUMMARY claims) for all 5 ROADMAP success criteria and all 9 CLEANUP-05..13 requirement IDs; full monorepo suite green (server 1635, client 1248 — a known Windows vitest worker-crash flake on the plain `pnpm test` invocation was confirmed unrelated by rerunning with `--pool=forks`); code review 0 critical, 3 warning (duplicated end-turn-confirm dialog logic, dead CSV-parsing branch in `seed-rosters.ts`, a silent invalid-jerseyType coercion), 1 info. This completes all 6 phases (41–46) originally scoped for v1.7 UI Consistency, Substitution Rework & Match Summary — milestone close-out via `/gsd-complete-milestone` is the natural next step. Two items surfaced during phase-close were not auto-filed and are worth a manual look: a narrow SHOT-phase goalie-response gap noted in the CLEANUP-07 audit was never filed to `.planning/todos/pending/` despite being legitimately out-of-scope (it's already tracked as backlog Phase 999.1); and the pre-existing `2026-08-28-debt-move-speed-setting-to-advanced-settings.md` todo — which this phase's Match Speed relocation resolves — has no `resolves_phase` frontmatter field, so it wasn't auto-closed with the other four Phase 46 todos.

**v1.7 in progress — Phase 45 (Game Summary Popup) complete 2026-08-29.** Fifth of 6 phases (6 plans, 4 waves, 9/9 requirements — STATS-01..09 — satisfied). Delivered an on-demand match summary reachable from an always-clickable (i) icon on the scoreboard, plus the same shared `MatchSummaryContent` block appended inside the existing HALF_TIME/FULL_TIME overlays: a settings/toggle recap (six match-rule toggles as colored red/green bubbles, Referee Leniency green only when left at its Auto default, Speed shown with its own hue) and eight per-team stat rows (possession, completed passes, tackles+steals with success %, shots, accumulated xG, fouls, yellow cards, red cards) as diverging comparison bars. xG is captured via the user's exact formula at all seven independent shot-resolution call sites (standard shot, snapshot/deflection, headed shot, penalty, GK-dive-at-feet penalty) with a pure, unit-tested formula module; all nine counters persist across half-time exactly like the existing `subsUsed` pattern, proven end-to-end by a real two-client socket integration suite. Checkpoint 45-05-04 went through 4 rounds of live two-browser developer verification — the largest single-checkpoint iteration count since Phase 38's Corner Kick — fixing a red-card false-positive free-kick zone-check bug, a mangled phase-label string, modal chrome (removed corner ×, restyled the Close button, added a score row), then extracting a single shared `MatchScoreRow` component reused identically by both the standalone modal and the half-time/full-time header (there was no spec constraint forcing them to differ — an unstated round-1 judgment call), centering the settings-bubble grid, and finally flipping the Leniency bubble's color rule to green-on-default. Verified independently: 9/9 must-haves re-checked against live source, full monorepo suite green (shared 902 / server 1635 / client 1212), code review 0 critical/blocker (2 warnings, 1 info). Three items surfaced during live verification were explicitly deferred rather than fixed in-phase: a shot-from-outside-the-box goalie-response gap (backlog, Phase 999.1), PK/FK kicker-selection pattern alignment (already in Phase 46's planned scope), and moving the Speed setting into the Advanced Settings drawer (new Phase 46 todo).

**v1.7 in progress — Phase 43 (Tackle/Steal Prompt & Decline) complete 2026-08-23.** Third of 6 phases (6 plans, 6 waves, 4/4 requirements — TACKLE-01..04 — satisfied). Delivered a server-authoritative decline mechanic: a new `TACKLE_STEAL_PROMPT` phase intercepts every tackle/steal duel when a game-creation toggle (default on) is set, holding the defending manager's Attempt/Decline choice before dice resolve; multiple eligible defenders are prompted sequentially in tackling-descending order (highest first); declining never counts as an attempt, so the amber ZoI risk ring stays live for that defender until the ball carrier leaves range or the movement phase ends; with the toggle off, duels resolve immediately exactly as before. Zero pre-existing tackle/steal/foul test files were modified across all six plans — the strongest available evidence for the toggle-off regression requirement (TACKLE-04), independently confirmed via `git diff --stat` against the phase base commit. Verified independently: 10/10 must-haves re-checked against live source (not SUMMARY.md claims), full monorepo suite green (shared 886 / server 1552 / client 1141), `pnpm -r typecheck` and `pnpm knip` clean, live two-browser walkthrough approved by the developer. Code review: 0 blockers, 2 warnings (both confirmed intentional/benign), 2 info. One out-of-scope UX gap surfaced during the live walkthrough — the moved piece isn't auto-reselected with its movement ring after any interrupt-style prompt resumes play (pre-existing across GK dive-at-feet/box-entry/foul-choice too, not a Phase 43 regression) — filed to `.planning/todos/pending/` for Phase 46 (Final Cleanup).

**v1.7 in progress — Phase 41 (Card & Injury Iconography) complete 2026-08-21.** First of 6 phases (6 plans, 12 tasks, 3/3 requirements — ICON-01, ICON-02, ICON-03 — satisfied). Delivered a single shared `CardInjuryBadge` component (colored-rectangle card glyph + white-cross injury glyph) that replaced three independently-implemented treatments (pitch piece overlay, player-stats card, roster/lineup card) and extended card/injury display to the bench for the first time — the bench previously showed neither. `BenchEntry` gained `yellowCards`/`injuryCount` fields, propagated on substitution and send-off. A permanent source-scanning audit test (`CardInjuryBadge.audit.test.ts`) and a four-surface consistency test (`CardInjuryBadge.crossSurface.test.tsx`) close the loop against regression. Verified independently: all four surfaces confirmed importing the shared component, zero hex-color literals, 196 phase-specific tests plus the full monorepo suite (shared/server/client) green. Code review: 0 critical, 2 warning (nested ARIA labeling, an audit-test regex fragility), 1 info — all non-blocking.
...
- Test suite (v1.6 close): 839 shared + 1439 server (1 skipped, 1 todo) + 998 client = 3,276 tests total, all green
- Known tech debt entering next milestone: RESP-01..09 (response-move activation, still top priority, now deferred across three consecutive milestones); an open, user-paused foul→injury→booking EventBanner sequencing bug (a real same-commit React state race was found and fixed but does not resolve the live-reported symptom — root cause still unconfirmed, see `.planning/debug/foul-banner-sequence-not-pausing.md`); 2 non-blocking Phase 38 code-review warnings (ActionLog switch exhaustiveness gap, corner-kick occupancy-check client/server mismatch) and 2 non-blocking Phase 40 warnings (cosmetic GK-parity rejection copy, inert missing dragover guard); Nyquist VALIDATION.md staleness on Phases 38/39/40 (pre-execution planning docs never refreshed post-execution — documentation-currency gap only, actual test coverage independently confirmed extensive); 3 unverified Phase 34 code-review findings (lineup-grid overflow, invalid-hex jersey-toggle fallback, unanchored `check-contrast.ts` regex); `buildTierPoolsForRound` over-claims unclaimed primary-tier population beyond `need` (non-correctness-impacting, Phase 36 WR-02); `createServer.ts:99-167` reconnect handler-registration bug (pre-existing since Phase 07, flagged repeatedly, unremediated); KICK_OFF_SETUP shot-path shading persists on some SNAPSHOT_DEFLECT-goal paths (root cause still unknown, `.planning/todos/pending/`); offside-ring-after-goal (triaged out of Phase 38 scope with written evidence, `.planning/todos/pending/`); `.planning/todos/pending/csv-consolidation-player-pool.md` (low-priority idea)
...
_Last updated: 2026-08-30 after Phase 46 (Final Cleanup) completion_
