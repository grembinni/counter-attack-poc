...
- **D-13 [deferred]:** ActionPanel.tsx pulled into DESIGN-01 scope, but not yet planned — tagged deferred until the user reviews the trigger/text/actions inventory and corrects target text the same way they did for the scoreboard table (D-11), at which point plan 18-03 will be written and this tag removed. The original DESIGN-01 requirement text ("action panel labels, phase prompts...") was locked to only the `PHASE_LABEL` map + log formats when 18-01/18-02 were planned (D-01 explicitly scoped to `GameBoard.tsx:17-43`). The user has now pulled `ActionPanel.tsx`'s ~14 distinct phase-prompt/helper-text states into scope as well — same naming convention (TEAM + plain VERB/NOUN where applicable, fix stale/inconsistent wording per D-04, e.g. `HIGH_PASS_MOVE`'s helper text says "Header!" before a header has actually been triggered — likely confusing/stale and worth fixing inline). This requires a new plan (`18-03`) covering `ActionPanel.tsx` exclusively (no `files_modified` overlap with 18-01/18-02). The full current-state inventory of every trigger/helper-text/available-actions state in `ActionPanel.tsx` was produced and is the working input for that plan — see `18-PATTERNS.md` (to be updated) or the orchestrator's conversational response for the full table; target text per state is Claude's Discretion (apply the same convention rules already established) unless the user supplies literal corrections the same way they did for the scoreboard table above.
