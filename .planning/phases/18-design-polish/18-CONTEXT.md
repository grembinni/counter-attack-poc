...
### DESIGN-01 Addendum (2026-06-20, post-plan-review corrections)

After plan 18-01/18-02 were drafted, the user reviewed the proposed scoreboard target list and corrected several values, and clarified the log-format unification and scope.

- **D-11:** Scoreboard target corrections — supersede the matching values planned in 18-01 Task 1.

  | GamePhase              | Superseded plan value        | Corrected target (LOCKED)                                                                                                    |
  | ---------------------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
  | `PASS`                 | `CHOOSE ACTION`              | `CHOOSING ACTION` (revert — D-01's gerund-removal example for this specific phase is walked back; gerund is acceptable here) |
  | `GK_DIVE`              | `GK DIVE`                    | `GOALIE DIVE` (`GK` → `GOALIE` expansion)                                                                                    |
  | `SNAPSHOT_TARGET`      | `SHOT TARGET`                | `SNAPSHOT - SELECT TARGET`                                                                                                   |
  | `SNAPSHOT_DEFLECT`     | `SNAPSHOT DEFLECT`           | `SNAPSHOT - RESPONSE MOVE`                                                                                                   |
  | `GK_RESTART`           | `GK RESTART`                 | `GOALIE RESTART`                                                                                                             |
  | `GK_KICK_TARGET`       | `GK KICK TARGET`             | `GOALIE KICK — SELECT TARGET`                                                                                                |
  | `GK_KICK_MOVE`         | `GK KICK REPOSITION`         | `GOALIE KICK — REPOSITION`                                                                                                   |
  | `FIRST_TIME_PASS_MOVE` | `FIRST TIME PASS REPOSITION` | `FIRST-TIME PASS — RESPONSE MOVE`                                                                                            |
  | `FREE_KICK_SETUP`      | `FREE KICK SETUP`            | `OFFSIDES - FREE KICK SETUP`                                                                                                 |

  All other 18-01 target values (KICKOFF, KICKOFF SETUP, MOVE, SHOT, HEADER, SNAPSHOT, LOOSE BALL, HIGH PASS REPOSITION, QUICK THROW, FREE MOVE ATTACK/DEFENSE, HALF TIME, FULL TIME, REPLAY) are unchanged. Note the asymmetry: `HIGH_PASS_MOVE` and `GK_KICK_MOVE` keep `REPOSITION` while `SNAPSHOT_DEFLECT` and `FIRST_TIME_PASS_MOVE` become `RESPONSE MOVE` — confirmed intentional (response-to-opponent-action phases get "RESPONSE MOVE"; the GK's own kick continuation keeps "REPOSITION"). `GOALIE KICK` em-dash usage and `FIRST-TIME PASS` hyphenation are also restored from the original GameBoard.tsx source (plan 18-01 had flattened these to plain spaces — that flattening is reverted).

- **D-12:** Unified stat+roll+penalty log format — supersedes/sharpens D-03's "apply uniformly" instruction. `SHOT_ATTEMPT`, `TACKLE_ATTEMPT`, `STEAL_ATTEMPT`, and `HEADER` must all render the SAME spelled-out structure — no compact `(die+stat=score)` parenthetical shorthand. Use the literal D-03 example shape: `{Stat Name} {statValue} + {roll} - {penalty} = {combined}`. Per-event stat mapping (confirmed from `gameEngine.ts`):
  - `SHOT_ATTEMPT`: shooter side uses `Shooting`; GK side uses `Saving` (handling sub-check, when it runs, is unchanged — not part of this stat+roll+penalty triple).
  - `TACKLE_ATTEMPT`: defender side uses `Tackling`; carrier side uses `Dribbling`. Both rolls have no penalty field today (penalty is always 0 in current code) — still render the `- 0` term per D-03.
  - `STEAL_ATTEMPT`: defender side uses `Tackling` (no carrier side — single roll only). Penalty is always 0 today — still render `- 0`.
  - `HEADER`: both attacker and defender sides use `Aerial Ability`.
    This replaces 18-02 Task 1's "apply fmtScore to each case, keep the parenthesized layout" instruction — the parenthesized compact layout is dropped in favor of the spelled-out uniform structure for these 4 types only. `DICE_ROLL` remains exempt (no stat triple) per the original D-03 carve-out. `DEFLECT_ATTEMPT` is not in this list — no change.

- **D-13:** ActionPanel.tsx pulled into DESIGN-01 scope (originally locked to only `PHASE_LABEL` + log formats per D-01's `GameBoard.tsx:17-43` scoping). The user reviewed the full trigger/helper-text/available-actions inventory of `ActionPanel.tsx` and supplied literal target text, covered by plan `18-03` (new file, no `files_modified` overlap with 18-01/18-02). LOCKED target table:

  | State                                                                                    | Trigger                                             | Line 1                                                      | Line 2                                                                                                                                                                                                                                                    | Actions                                                                                                                 |
  | ---------------------------------------------------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
  | 1                                                                                        | HIGH_PASS_MOVE, active player                       | `High Pass Aerial Challenge!`                               | `Move 1 player to challenge (max 3 hexes).`                                                                                                                                                                                                               | Undo (if available), End Turn                                                                                           |
  | 2                                                                                        | FIRST_TIME_PASS_MOVE, active player                 | `First-Time Pass!`                                          | `Move 1 player to receive the ball (max 1 hex).`                                                                                                                                                                                                          | Undo (if available), End Turn                                                                                           |
  | 3                                                                                        | GK_DIVE, GK's team                                  | `Attempt Save!`                                             | `Dive to a highlighted hex (max 3 hexes).`                                                                                                                                                                                                                | none (click-to-target)                                                                                                  |
  | 4                                                                                        | SNAPSHOT_DEFLECT, defending team                    | `Snapshot - Deflection Attempt!`                            | `Move 1 player to deflect the shot (up to 2 hexes).`                                                                                                                                                                                                      | End Turn                                                                                                                |
  | 5a                                                                                       | HEADER, accuracy-roll pending, active attacker      | `Accurate High Pass!`                                       | `Click to continue.`                                                                                                                                                                                                                                      | Continue                                                                                                                |
  | 5c                                                                                       | HEADER, both confirmed, duel won                    | `Header Won!`                                               | `Select a target hex.`                                                                                                                                                                                                                                    | none (click-to-target)                                                                                                  |
  | 5e                                                                                       | HEADER, not yet confirmed, pending                  | `Contest Header!`                                           | `{n} players selected within range.`                                                                                                                                                                                                                      | Confirm Selection (n>0) / Decline (n=0)                                                                                 |
  | 6                                                                                        | SNAPSHOT_TARGET, active player                      | `Snapshot!`                                                 | `Select a goal hex to target.`                                                                                                                                                                                                                            | none (click-to-target)                                                                                                  |
  | 7                                                                                        | GK_RESTART, GK's team                               | `Goalie Restart!`                                           | `Choose an action.`                                                                                                                                                                                                                                       | **`Punt (High Pass)`** (renamed from `Kick (High Pass)` — terminology now matches state 9's "Punt!"), Quick Throw, Move |
  | 8                                                                                        | GK_QUICK_THROW, GK's team                           | `Quick Throw!`                                              | `Select a target hex (up to 11 hexes).`                                                                                                                                                                                                                   | none (click-to-target)                                                                                                  |
  | 9                                                                                        | GK_KICK_TARGET, GK's team                           | `Punt!`                                                     | `Target anywhere except the opponent's final third.`                                                                                                                                                                                                      | none (click-to-target)                                                                                                  |
  | 10                                                                                       | GK_KICK_MOVE, active player                         | `Ball in Air!`                                              | `Move 1 player to receive the ball (max 3 hexes).`                                                                                                                                                                                                        | End Turn                                                                                                                |
  | 11                                                                                       | FREE_MOVE_ATTACK / FREE_MOVE_DEFENSE, active player | `Free Move!`                                                | `{Attacking/Defending} team — move up to 6 hexes per player in the opponent's third.`                                                                                                                                                                     | End Turn                                                                                                                |
  | 12a                                                                                      | PASS, carrier null (loose ball)                     | `Loose Ball!`                                               | `Move to collect.`                                                                                                                                                                                                                                        | Move                                                                                                                    |
  | 12b                                                                                      | PASS/KICK_OFF, no pass type selected                | `Choose Action`                                             | _(none)_                                                                                                                                                                                                                                                  | Move, Standard Pass, One-Touch, High Pass, Long Ball, Snapshot, Shoot, Cancel Shot (dynamic by eligibility — unchanged) |
  | 12c                                                                                      | PASS/KICK_OFF, pass type selected, no target yet    | `{Pass Type Label}`                                         | `Select a target hex.`                                                                                                                                                                                                                                    | Back                                                                                                                    |
  | 12d                                                                                      | PASS/KICK_OFF, target hex clicked                   | _(renders nothing — UNCHANGED, no new "Auto-rolling" text)_ |                                                                                                                                                                                                                                                           | none                                                                                                                    |
  | 13                                                                                       | MOVE, active player                                 | `Move!`                                                     | `Move up to {4/5/2} players.` + **conditionally, ONLY when `movementSlot === 'ATTACKER_2'`**: `(2 hex max)` — ATTACKER_4/DEFENDER_5 show no second line (each piece uses its own `pace` stat, not a fixed number; confirmed via `moveValidator.ts:87-89`) | Snapshot (if eligible), Undo, End Turn, Back (ATTACKER_4, before pace used)                                             |
  | 14                                                                                       | any phase, active player, with a game error         | _(error text only — unchanged)_                             |                                                                                                                                                                                                                                                           | none                                                                                                                    |
  | 15                                                                                       | fallback                                            | _(nothing — unchanged)_                                     |                                                                                                                                                                                                                                                           | none                                                                                                                    |
  | ALL non-active-player wait states (1w, 2w, 3w, 4w, 5b, 5d, 5f, 6w, 7w, 8w, 9w, 10w, 11w) | —                                                   | `Opponent's Turn`                                           | `Waiting for opponent...`                                                                                                                                                                                                                                 | none                                                                                                                    |

  Confirmed decisions on the open questions: (a) wait-state unification is intentional — 5b's "Accurate High Pass!" context is deliberately dropped in favor of full consistency; (b) state 12d stays exactly as today (renders nothing) — no new auto-roll feedback text, that would be new behavior out of scope for a naming-convention sweep; (c) "Punt" is the winning term — the GK_RESTART button is RENAMED from `Kick (High Pass)` to `Punt (High Pass)` so the term is consistent wherever this mechanic is referenced (internal `emitGKRestart('kick')` call/type value is unchanged — this is a display-text-only rename); (d) action-panel prompt text does NOT need to mirror the scoreboard's GOALIE convention — it's fine for `Quick Throw!`/`Punt!`/`Ball in Air!` to omit "Goalie" while `Goalie Restart!` includes it.

- **D-05 [deferred]:** (Relocated to Phase 18.1 — tracked in `18.1-CONTEXT.md`; out of scope for Phase 18's own plans.) No specific symptom was reported for DESIGN-02. Treat it as an open-ended, clean-room audit of `startReplayStream`/`roomStore.ts` (server) and `ReplayPanel.tsx` (client) for unnecessary re-renders or redundant socket emissions, observable via React DevTools profiler / browser Network tab. Document findings before fixing if the root cause isn't obvious from a first read.
- **D-06 [deferred]:** (Relocated to Phase 18.1 — tracked in `18.1-CONTEXT.md`; out of scope for Phase 18's own plans.) REPLAY-06 (live-session ball tracking — pickups, intercepted passes, steals mid-replay) likewise has no new repro beyond what's already in REQUIREMENTS.md/PROJECT.md ("Known tech debt: REPLAY-06 live-session ball tracking edge cases deferred"). This remains an open investigation — read `.planning/phases/14-kick-off-rules-replay/` artifacts (replay was introduced/iterated there) before starting.
...
- **D-08 [deferred]:** (Relocated to Phase 18.2 — tracked in `18.2-CONTEXT.md`; out of scope for Phase 18's own plans.) Stay limited to genuinely inert code for everything else flagged by DESIGN-03/04: unused exports, unreachable branches, stale TODO comments, legacy feature flags. Do not go looking for additional behavioral bugs beyond what's explicitly listed in this CONTEXT.md's bug-bash section — that scope is closed as of this discussion.
- **D-09 [deferred]:** (Relocated to Phase 18.2 — tracked in `18.2-CONTEXT.md`; out of scope for Phase 18's own plans.) The 4 pre-existing RED test failures documented in STATE.md (2 MOVE-06 FREE_MOVE scaffolding gaps, 2 abandoned firstTimePassStep design stubs) remain explicitly out of scope — they were deliberately deferred past Phase 17.1, and nothing in this discussion changes that.
