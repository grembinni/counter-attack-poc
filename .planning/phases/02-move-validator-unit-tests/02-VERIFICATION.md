...
human_verification_signoffs:
  - test: 'Verify Loose Ball direction mapping against physical Counter Attack rulebook v1.4.1 deflection ruler'
    ...
    note: 'Loose ball scenarios played through Phase 10 UAT without directional issues. Physical rulebook check deferred to v1.1 as a known assumption. Direction mapping (1=E, 2=NE, 3=NW, 4=W, 5=SW, 6=SE) is consistent and internally valid.'
  - test: 'Verify pass attribute mapping: High Pass uses aerialAbility, Long Pass uses dribbling'
    disposition: accepted
    note: 'Phase 2 wording is stale. D-04 correction in Phase 8.2 already changed Long Pass attribute from dribbling to piece.highPass — same attribute as High Pass. Both pass types now use piece.highPass. Whether the rulebook confirms highPass for Long Pass is deferred to v1.1.'
---
...
#### 1. Loose Ball Direction Mapping — Accepted

Loose ball scenarios played through Phase 10 UAT (10/10 tests pass, commit `4a1ff80`) without directional gameplay issues. The mapping (1=E, 2=NE, 3=NW, 4=W, 5=SW, 6=SE) in `scoreUtils.ts:48–55` is internally consistent. Physical rulebook comparison deferred to v1.1 as a known assumption.

#### 2. Pass Attribute Mapping — Accepted (Phase 2 wording stale)

Phase 2 expected `aerialAbility`/`dribbling`. D-04 correction in Phase 8.2 changed Long Pass from `piece.dribbling` to `piece.highPass` — both High Pass and Long Pass now use `piece.highPass`. The code change predates Phase 10 UAT; passing E2E tests confirm this is the working implementation. Physical rulebook confirmation of `highPass` for Long Pass deferred to v1.1.
