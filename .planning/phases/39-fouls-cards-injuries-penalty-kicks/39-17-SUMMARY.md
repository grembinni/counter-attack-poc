...
**Task 3 ran — live two-browser UAT found 9 gaps, not yet approved.** The human tested the live app (server + client dev, two browser windows) and reported 9 issues via ad-hoc exploration rather than a clean pass through all 32 numbered steps: 8 real behavior bugs (continue-or-restart offered after a successful+fouled duel; restart location using the fouler's hex instead of the ball's hex; missing hex-selection UX on GK dive-at-feet; the dive-at-feet/shot-block-dive shared cap not holding within one movement phase; penalty kick trigger scoped to GK-dive fouls only instead of all in-box fouls, missing the full box-clearance/kicker-lock/shoot-only mechanic; no penalty-kicker select-and-confirm UX; a new tackle-from-behind rule entirely absent; and the professional-foul/DOGSO geometry using omnidirectional teammate-coverage instead of the intended horizontal-proximity-to-goal + goal-path-reachability test) plus 1 scope change (corner/free/penalty kicks should each cost 1 minute, matching the goal kick, instead of 0). Full detail, code pointers, and YAML gap records (feeding `/gsd-plan-phase 39 --gaps`) are in `.planning/phases/39-fouls-cards-injuries-penalty-kicks/39-UAT.md`.

No requirement in this plan's frontmatter (`FOUL-01..04`, `CARD-01..03`, `INJURY-01..03`, `GKDIVE-02..05`, `PEN-01..03`, `FK-01`, `SETTINGS-01..03`) can be marked SATISFIED until gap-closure lands and a fresh live-session pass is explicitly approved by a human — the executor did not and must not simulate or fabricate this verification. Phase 39 is NOT complete; do not mark it done in ROADMAP.md/STATE.md.
