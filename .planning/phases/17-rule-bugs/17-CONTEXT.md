...
- **D-58 (halfway-line confirmation — no code change, D-21 already correct):** User restated: "A player cannot be offside on their half of the field (with the goal they defend), include the middle line." Re-verified against the live `isPastHalfway` implementation in `packages/shared/src/offside.ts`: `(piece.position.q - OFFSIDE_HALFWAY_Q) * dir > 0` already returns `false` both for a piece strictly on its own half AND for a piece exactly on the q=18 boundary (zero is not `> 0`) — the existing implementation already satisfies this exactly as stated. No fix needed; documented here for traceability since the user raised it explicitly.
- **D-59 (BUG FIX — the offender must also be auto-relocated, not left on the kick spot; relocation algorithm refined):** D-53's auto-relocation explicitly EXCLUDED the offending player ("they are the kick spot marker, so they're never moved") — this is the actual bug: since the offender is a CONCEDING-team piece sitting exactly ON `freeKickHex`, and the kicking team must place one of their OWN pieces on that same hex (D-31/D-54), the hex stays permanently occupied by the offender and the kicking team can never legally complete the mandatory kicker-first placement — the game stalls. Fix: the offender is now INCLUDED in the same auto-relocation sweep as every other conceding-team piece within 2 hexes of `freeKickHex` (D-53), freeing the kick spot before stage 0 ever becomes interactive. Additionally, refine the relocation TARGET selection for every relocated piece (not just the offender): prefer the 4 hexes at EXACTLY distance 3 from `freeKickHex` that are closest to the defending (conceding) team's own goal line (q=0 r∈[10,16] for home, q=36 r∈[10,16] for away — the same goal-line hex set used elsewhere, e.g. `applyDeclareShot`'s `goalQ`/`GOAL_R_VALUES`), tried in closest-first order, skipping any already occupied; if none of those 4 are available (all occupied), fall back to D-53's original behavior — pick a random unoccupied on-pitch hex that is `hexDistance >= 3` from `freeKickHex` (`crypto.randomInt`, not `Math.random`). This keeps relocated defenders tactically sensible (pushed back toward their own goal) rather than scattering them randomly across the whole pitch by default.
