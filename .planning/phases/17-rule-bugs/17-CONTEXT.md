...
- **D-52 (header won by an offside player — skips the redirect step entirely):** If the piece that wins a HEADER duel (`headerDuelWinner`'s resolved contestant) is already in `offsidePieceIds`, do NOT enter the target-selection step (no `GAME_HEADER_TARGET` choice of where to redirect the header). Immediately trigger the free-kick foul (reusing the generalized `triggerOffsideFoul(state, explicitOffenderId)` from D-41, called with the winner's id) and transition straight to `FREE_KICK_SETUP`. This applies regardless of what the winner would have aimed at (a headed pass or a headed shot at goal) — winning the duel while offside is itself the foul; there is no "pass" to skip past, you go straight to the consequence.
