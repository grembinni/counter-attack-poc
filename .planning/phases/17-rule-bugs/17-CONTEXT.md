...
- **D-46 (new defending-team placement constraint — extends D-30):** In addition to D-30's existing 2-hex exclusion zone around the restart hex, the defending team (the side that committed the offside foul) may not position ANY piece BEHIND the ball — i.e., on their own defensive side of `freeKickHex` in their own attacking direction (the same `attackingDirection`/team-relative convention as D-21/D-24's offside geometry: home attacks toward higher q, away toward lower q). Equal-to-or-ahead of the ball (toward the opponent's goal, in the defending team's own attacking direction) is legal; strictly behind it is not. Rationale (as given): getting caught offside denies the offending team the chance to retreat into a defensive shape — they're forced to stay pushed up. The kicking team (D-29) has NO such restriction — they may place anywhere on the board. Enforce server-side in `applyFreeKickReady` (a new `DEFENDER_BEHIND_BALL` rejection reason, checked alongside the existing `DEFENDER_TOO_CLOSE` per-piece loop) and reflect it in the client's valid-hex computation (D-45) so the UX hint matches what the server will actually accept.
