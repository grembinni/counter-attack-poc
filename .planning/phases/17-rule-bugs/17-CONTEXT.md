...
- **D-51 (kicker-hex check timing — refines D-31, not superseded):** D-31's "kicking team must have exactly one piece on `freeKickHex`" is validated when the kicking team attempts to end stage 3 (their last turn, `KICKER_HEX_EMPTY` if not satisfied) — not at stage 1, since they may legitimately wait until stage 3 to finalize the kicker, and not after stage 4, since by then they have no further turns to fix it.
