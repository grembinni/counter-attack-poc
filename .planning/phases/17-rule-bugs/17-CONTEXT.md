...
- **D-36 (resume phase):** Because the trigger can fire after _any_ resolved action (not just a MOVEMENT End Turn), add `freeMoveResumePhase: GamePhase | null` to `GameState`. When the trigger fires, capture the phase the triggering action had already computed as "next" into `freeMoveResumePhase`, then overwrite `phase` with `'FREE_MOVE_ATTACK'`. When `FREE_MOVE_DEFENSE` ends, restore `phase: state.freeMoveResumePhase`, clear `freeMoveResumePhase`/`ballZone`-working fields (`freeMoveEligibleIds`, `freeMoveUsedPace`), but leave `ballZone` itself set to the zone that fired the trigger (so it doesn't immediately refire).
...
- **D-38 (types):** `GamePhase` union replaces `'FREE_MOVE'` with `'FREE_MOVE_ATTACK' | 'FREE_MOVE_DEFENSE'`. `GameState` drops `pendingFreeMove` (carrier-cross detection no longer used) and gains `ballZone: 'home' | 'middle' | 'away'`, `freeMoveResumePhase: GamePhase | null`, and an eligible-ids shape that can hold both teams' precomputed lists (e.g. `freeMoveEligibleIds: { attack: string[]; defense: string[] } | null`); `freeMoveUsedPace` keyed by piece id is shared across both sub-phases and reset at trigger time.
