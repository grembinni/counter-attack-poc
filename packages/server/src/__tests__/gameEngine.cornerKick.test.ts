...
describe('triggerOutOfBoundsRestart CORNER_KICK branch (OOB-03)', () => {
  it('awards a corner to AWAY when a HOME byline exit follows a HOME (defending) touch — team inversion, entering the mandatory clear-out (38-20)', () => {
    ...
    // CORNER-01 (gap-closure round 3, 38-25): the clear-out is now automatic — a corner
    // opens directly on the attacking goalkeeper reposition window, no intermediate phase.
    expect(result!.phase).toBe('CORNER_KICK_GK_SETUP_ATTACKING');
  });

  it('awards a corner to HOME when an AWAY byline exit follows an AWAY (defending) touch — mirror case, opening directly on the goalkeeper window (38-25)', () => {
    const state: GameState = {
      ...baseLooseBallState,
      ball: {
        position: { q: 35, r: 13 },
...
/** 38-27: extra away outfield pieces for the A-GAP3-BUDGET six-distinct-pieces test. */
const awayPiece4: PlayerPiece = { ...awayPiece, id: 'away-6', position: { q: 12, r: 18 } };
const awayPiece5: PlayerPiece = { ...awayPiece, id: 'away-5', position: { q: 14, r: 18 } };
const awayPiece6: PlayerPiece = { ...awayPiece, id: 'away-4', position: { q: 16, r: 18 } };
...
describe('applyCornerKickReposition', () => {
  ...
  // 38-27: adjacency is gone entirely — this is the free-kick-style model, single
  // destination click at any distance inside the bounded allowed area.
  it('accepts a placement at a hex more than one away from the current position and moves the piece exactly there', () => {
    const to = { q: 30, r: 16 };
    expect(hexDistance(awayPiece.position, to)).toBeGreaterThanOrEqual(5);
    const result = applyCornerKickReposition(baseCornerRepositionState, awayPiece.id, to);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it('on success moves the piece, increments cornerKickUsedPace by hexDistance(from, to) — not a flat 1 — and adds pieceId to both cornerKickStagePlacedIds and cornerKickActivatedIds', () => {
    const to = { q: 22, r: 16 };
    const expectedDistance = hexDistance(awayPiece.position, to);
    ...
    const moved = result.state.pieces.find((p) => p.id === awayPiece.id)!;
    expect(moved.position).toEqual(to);
    expect(result.state.cornerKickUsedPace).toEqual({ [awayPiece.id]: expectedDistance });
    expect(result.state.cornerKickStagePlacedIds).toEqual([awayPiece.id]);
    expect(result.state.cornerKickActivatedIds).toEqual([awayPiece.id]);
  });
  ...
  // 38-27: activation is applied the INSTANT a placement completes, so a same-piece
  // re-touch is rejected even within the SAME stage that activated it — the prior
  // same-stage re-touch exemption is gone.
  it('rejects PIECE_LOCKED on a second placement attempt of the same piece within the SAME stage', () => {
    const first = applyCornerKickReposition(baseCornerRepositionState, awayPiece.id, {
      q: 17,
      r: 16,
    });
    expect(first.ok).toBe(true);
    if (!first.ok) return;
    const second = applyCornerKickReposition(first.state, awayPiece.id, { q: 18, r: 16 });
    expect(second).toEqual({ ok: false, reason: 'PIECE_LOCKED' });
  });

  it('rejects STAGE_LIMIT_REACHED for a third distinct piece once the stage cap is reached; re-attempting one of the first two is PIECE_LOCKED, not a free re-touch', () => {
    ...
    expect(reMove).toEqual({ ok: false, reason: 'PIECE_LOCKED' });
  });
  ...
  it('applyCornerKickReposition: the stage cap comes from CORNER_KICK_STAGES, not a literal 2', () => {
    ...
    // Place exactly `expectedCap` distinct pieces so cornerKickStagePlacedIds reaches
    // precisely the table-derived cap.
    ...
    // 38-27: an already-placed piece is now PIECE_LOCKED, not free to move again — the
    // stage cap no longer has a re-touch exemption.
    const alreadyPlacedId = placedIds[0]!;
    const alreadyPlacedPiece = state.pieces.find((p) => p.id === alreadyPlacedId)!;
    const lockedRetry = applyCornerKickReposition(state, alreadyPlacedId, {
      q: alreadyPlacedPiece.position.q + 1,
      r: alreadyPlacedPiece.position.r,
    });
    expect(lockedRetry).toEqual({ ok: false, reason: 'PIECE_LOCKED' });
  });
});

// ---------------------------------------------------------------------------
// 38-27 (gap closure round 3, corrected reading of D-GAP-03 per 38-24-SUMMARY.md
// bug 2): bounded single-destination placement + activation applied the instant a
// placement completes, locking a piece for the REST of the whole reposition window
// (not just later stages).
// ---------------------------------------------------------------------------

describe('38-27: bounded single-destination placement + activation on completion', () => {
  ...
  // -------------------------------------------------------------------------
  // A-GAP3-BUDGET: CORNER_KICK_STAGES keeps its existing shape — the net budget
  // (2 distinct pieces per stage, 6 distinct pieces per side across its 3 stages) is
  // unchanged by 38-27; only the interaction model and activation timing changed.
  // -------------------------------------------------------------------------

  it('places six distinct attacking pieces across the three attacking stages (2 per stage); a seventh distinct piece is rejected STAGE_LIMIT_REACHED', () => {
    const budgetTestState: GameState = {
      ...baseCornerRepositionState,
      pieces: [...baseCornerRepositionState.pieces, awayPiece4, awayPiece5, awayPiece6],
      cornerKickEligibleIds: {
        attacking: [
          ...baseCornerRepositionState.cornerKickEligibleIds!.attacking,
          awayPiece4.id,
          awayPiece5.id,
          awayPiece6.id,
        ],
        defending: baseCornerRepositionState.cornerKickEligibleIds!.defending,
      },
    };
    let state = budgetTestState;

    // Stage 0 (attacking): place 2 distinct pieces.
    const s0a = applyCornerKickReposition(state, awayPiece.id, { q: 17, r: 16 });
    expect(s0a.ok).toBe(true);
    if (!s0a.ok) return;
    state = s0a.state;
    const s0b = applyCornerKickReposition(state, awayPiece2.id, { q: 11, r: 16 });
    expect(s0b.ok).toBe(true);
    if (!s0b.ok) return;
    state = s0b.state;

    const end0 = applyCornerKickStageEnd(state, 'away');
    expect(end0.ok).toBe(true);
    if (!end0.ok) return;
    state = end0.state;
    const end1 = applyCornerKickStageEnd(state, 'home'); // stage 1 (defending), zero moves — legal (D-06)
    expect(end1.ok).toBe(true);
    if (!end1.ok) return;
    state = end1.state;
    expect(state.cornerKickStageIndex).toBe(2);

    // Stage 2 (attacking): place 2 more distinct pieces.
    const s2a = applyCornerKickReposition(state, awayPiece3.id, { q: 9, r: 16 });
    expect(s2a.ok).toBe(true);
    if (!s2a.ok) return;
    state = s2a.state;
    const s2b = applyCornerKickReposition(state, awayEdge.id, { q: 1, r: 13 });
    expect(s2b.ok).toBe(true);
    if (!s2b.ok) return;
    state = s2b.state;

    const end2 = applyCornerKickStageEnd(state, 'away');
    expect(end2.ok).toBe(true);
    if (!end2.ok) return;
    state = end2.state;
    const end3 = applyCornerKickStageEnd(state, 'home'); // stage 3 (defending), zero moves
    expect(end3.ok).toBe(true);
    if (!end3.ok) return;
    state = end3.state;
    expect(state.cornerKickStageIndex).toBe(4);

    // Stage 4 (attacking, third and final attacking round): place the last 2 distinct pieces.
    const s4a = applyCornerKickReposition(state, awayPiece4.id, { q: 13, r: 18 });
    expect(s4a.ok).toBe(true);
    if (!s4a.ok) return;
    state = s4a.state;
    const s4b = applyCornerKickReposition(state, awayPiece5.id, { q: 15, r: 18 });
    expect(s4b.ok).toBe(true);
    if (!s4b.ok) return;
    state = s4b.state;

    expect(state.cornerKickActivatedIds).toHaveLength(6);
    expect(state.cornerKickActivatedIds).toEqual(
    ...
    // A seventh distinct piece, still untouched, is rejected — stage 4's own cap of 2 is
    // already reached, and there is no further attacking stage in the window.
    const seventh = applyCornerKickReposition(state, awayPiece6.id, { q: 17, r: 18 });
    expect(seventh).toEqual({ ok: false, reason: 'STAGE_LIMIT_REACHED' });
  });
...
describe('applyCornerKickStageEnd', () => {
  ...
  // 38-27: cornerKickUsedPace is a pure Undo-refund ledger that persists across stage
  // advances (never reset), but the piece itself is permanently PIECE_LOCKED for the rest
  // of the whole reposition window the instant its single placement completes — there is
  // no same-stage re-touch and no re-activation in a later stage.
  it('Pitfall 4 (revised by 38-27): cornerKickUsedPace persists across stage advances, but the piece itself is PIECE_LOCKED once a later stage begins', () => {
    let state = baseCornerRepositionState;
    ...
    if (!move1.ok) return;
    state = move1.state;
    const expectedDistance = state.cornerKickUsedPace?.[awayPiece.id];
    ...
    expect(state.cornerKickUsedPace?.[awayPiece.id]).toBe(expectedDistance); // carried forward, not reset
    ...
    expect(state.cornerKickUsedPace?.[awayPiece.id]).toBe(expectedDistance); // still carried forward

    // awayPiece was activated back in stage 0 (its cornerKickStagePlacedIds was reset to []
    // on both advances, but cornerKickActivatedIds was not) — a further move is rejected
    // PIECE_LOCKED.
    ...
    expect(state.cornerKickUsedPace?.[awayPiece.id]).toBe(expectedDistance);
  });
...
describe('Corner-context persistence and teardown audit (Pitfall 3, T-38-14, 38-04 Task 3)', () => {
   ...
   *
   * 38-25: the clear-out is now automatic (`applyAutomaticCornerClearOut`), applied inside
   * `triggerOutOfBoundsRestart` itself — the corner opens directly on
   * CORNER_KICK_GK_SETUP_ATTACKING with no intermediate confirm step. None of this fixture's
   * pieces sit inside the exclusion zone, so the clear-out is a no-op move-wise here.
   *
...
describe('spilled save: the second route into a Corner Kick (38-14, corrected to direction-only by 38-23/D-GAP-02)', () => {
  ...
  it("BEFORE (38-14): asserted a corner via the scatter walk exiting on the keeper's own byline. AFTER (38-23): the same dice now award the corner via the direction-only check — proven by asserting bylineDice.direction is itself a spill-corner direction, independent of the scatter outcome", () => {
    ...
    // directly on CORNER_KICK_GK_SETUP_ATTACKING, no intermediate phase.
...
describe('D-GAP-02: direction-only corner award on a spilled save', () => {
  ...
  it('the direction-only rule does not fire for a SHOT duel-tie loose ball', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.state.phase).not.toBe('CORNER_KICK_GK_SETUP_ATTACKING');
    expect(result.state.cornerKickTeam).toBeFalsy();
  });
  ...
  it('the corner is awarded to the team OPPOSITE the spilling keeper', () => {
    ...
    if (homeResult.ok) {
      expect(homeResult.state.phase).toBe('CORNER_KICK_GK_SETUP_ATTACKING');
    ...
    if (awayResult.ok) {
      expect(awayResult.state.phase).toBe('CORNER_KICK_GK_SETUP_ATTACKING');
...
describe('automatic corner clear-out (gap-closure round 3, 38-25)', () => {
  ...
  /** A defending piece also inside the zone, on the opposite (home) side. */
  const clearOutHome: PlayerPiece = { ...homePiece, id: 'home-clearout', position: { q: 1, r: 2 } };
  ...
  it('fixture sanity: clearOutAway and clearOutHome are both inside the exclusion zone; the derived legal target is on-pitch', () => {
    expect(isWithinCornerExclusionZone(clearOutAway.position, clearOutCornerHex)).toBe(true);
    expect(isWithinCornerExclusionZone(clearOutHome.position, clearOutCornerHex)).toBe(true);
    expect(clearOutLegalTarget).toBeDefined();
    expect(isPitchHex(clearOutLegalTarget)).toBe(true);
  });

  const triggerState: GameState = {
    ...baseLooseBallState,
    pieces: [homePiece, awayPiece, homeGK, awayGK, clearOutAway, clearOutHome],
    ball: {
      position: { q: 1, r: 13 },
      carrierId: null,
      lastTouchedBy: { pieceId: homePiece.id, teamId: 'home' },
  ...
  it('a corner award lands directly in CORNER_KICK_GK_SETUP_ATTACKING with no intermediate clear-out phase', () => {
    const result = triggerOutOfBoundsRestart(triggerState, { q: -1, r: 13 }, { q: 0, r: 13 });
    expect(result).not.toBeNull();
    expect(result!.phase).toBe('CORNER_KICK_GK_SETUP_ATTACKING');
  ...
  it('no returned piece of either team is inside the exclusion zone of cornerKickHex', () => {
    ...
    for (const p of result!.pieces) {
      expect(isWithinCornerExclusionZone(p.position, result!.cornerKickHex!)).toBe(false);
    }
  });

  it('appends one CORNER_KICK_CLEAR_OUT_MOVE event per piece that was inside the zone before the call', () => {
    const inZoneCountBefore = triggerState.pieces.filter((p) =>
      isWithinCornerExclusionZone(p.position, clearOutCornerHex),
    ).length;
    expect(inZoneCountBefore).toBe(2); // clearOutAway + clearOutHome
    const result = triggerOutOfBoundsRestart(triggerState, { q: -1, r: 13 }, { q: 0, r: 13 });
    expect(result).not.toBeNull();
    const clearOutEvents = result!.eventLog.filter((e) => e.type === 'CORNER_KICK_CLEAR_OUT_MOVE');
    expect(clearOutEvents).toHaveLength(inZoneCountBefore);
  });

  it('every piece already outside the zone keeps its exact prior position', () => {
    const result = triggerOutOfBoundsRestart(triggerState, { q: -1, r: 13 }, { q: 0, r: 13 });
    expect(result).not.toBeNull();
    const outsideBefore = triggerState.pieces.filter(
      (p) => !isWithinCornerExclusionZone(p.position, clearOutCornerHex),
    );
    expect(outsideBefore.length).toBeGreaterThan(0);
    for (const before of outsideBefore) {
      const after = result!.pieces.find((p) => p.id === before.id)!;
      expect(after.position).toEqual(before.position);
    }
  });

  it('the appended clear-out events all appear AFTER the OUT_OF_BOUNDS event in eventLog', () => {
    const result = triggerOutOfBoundsRestart(triggerState, { q: -1, r: 13 }, { q: 0, r: 13 });
    expect(result).not.toBeNull();
    const oobIndex = result!.eventLog.findIndex((e) => e.type === 'OUT_OF_BOUNDS');
    expect(oobIndex).toBeGreaterThanOrEqual(0);
    const clearOutIndices = result!.eventLog
      .map((e, i) => (e.type === 'CORNER_KICK_CLEAR_OUT_MOVE' ? i : -1))
      .filter((i) => i >= 0);
    expect(clearOutIndices.length).toBeGreaterThan(0);
    for (const i of clearOutIndices) {
      expect(i).toBeGreaterThan(oobIndex);
    }
  });

  it('each clear-out event slots ATTACKER for the awarded team and DEFENDER for the conceding team', () => {
    const result = triggerOutOfBoundsRestart(triggerState, { q: -1, r: 13 }, { q: 0, r: 13 });
    expect(result).not.toBeNull();
    const cornerKickTeam = result!.cornerKickTeam!;
    const awayEvent = result!.eventLog.find(
      (e) => e.type === 'CORNER_KICK_CLEAR_OUT_MOVE' && e.pieceId === clearOutAway.id,
    );
    const homeEvent = result!.eventLog.find(
      (e) => e.type === 'CORNER_KICK_CLEAR_OUT_MOVE' && e.pieceId === clearOutHome.id,
    );
    expect(awayEvent).toMatchObject({
      slot: clearOutAway.teamId === cornerKickTeam ? 'ATTACKER' : 'DEFENDER',
    });
    expect(homeEvent).toMatchObject({
      slot: clearOutHome.teamId === cornerKickTeam ? 'ATTACKER' : 'DEFENDER',
    });
  });

  describe('applyAutomaticCornerClearOut (direct unit coverage)', () => {
    it('moves an in-zone piece to the same destination cornerClearOutDestination would compute', () => {
      const pieces = [homeGK, awayGK, clearOutAway];
      const result = applyAutomaticCornerClearOut(pieces, clearOutCornerHex, 'away');
      const expected = cornerClearOutDestination(
        clearOutAway.position,
        clearOutCornerHex,
        clearOutGoalHex,
        [homeGK.position, awayGK.position],
      );
      const moved = result.pieces.find((p) => p.id === clearOutAway.id)!;
      expect(moved.position).toEqual(expected);
    ...
    it('leaves a piece already outside the zone untouched with no event', () => {
      const pieces = [homeGK, awayGK];
      const result = applyAutomaticCornerClearOut(pieces, clearOutCornerHex, 'away');
      expect(result.events).toHaveLength(0);
      expect(result.pieces).toEqual(pieces);
    });

    it('later pieces in the array see the positions already produced by earlier pieces (order-stable occupancy)', () => {
      const pieces = [clearOutAway, clearOutHome];
      const result = applyAutomaticCornerClearOut(pieces, clearOutCornerHex, 'away');
      const positions = result.pieces.map((p) => `${p.position.q},${p.position.r}`);
      expect(new Set(positions).size).toBe(positions.length); // no two pieces share a hex
    });
  });
