...
// ---------------------------------------------------------------------------
// Task 2: GAME_FREE_KICK_MOVE / GAME_FREE_KICK_READY handlers
// ---------------------------------------------------------------------------

/**
 * Seeds a room directly into FREE_KICK_SETUP — as if triggerOffsideFoul had
...
describe('OFFSIDE-02 Task 2: GAME_FREE_KICK_MOVE handler', () => {
  it('repositions an owned piece anywhere on the pitch during FREE_KICK_SETUP', async () => {
    const { clientA, roomCode } = await setupRoom();
    ...
    const moved = newState.pieces.find((p) => p.id === homePiece.id);
    expect(moved?.position).toEqual({ q: 2, r: 2 });
    expect(newState.phase).toBe('FREE_KICK_SETUP');
  });

  it('rejects repositioning a piece not owned by the requesting socket', async () => {
    const { clientA, roomCode } = await setupRoom();
    seedFreeKickSetup(roomCode);
    const room = getRoom(roomCode);
    if (!room || !room.gameState) throw new Error('Room not found');
...
describe('OFFSIDE-02 Task 2: GAME_FREE_KICK_READY handler', () => {
  it('rejects Ready with DEFENDER_TOO_CLOSE when a defender is within 2 hexes of the ball', async () => {
    const { clientA, roomCode } = await setupRoom();
  ...
  it('rejects Ready with KICKER_HEX_EMPTY when the kicking team has no piece on the ball hex', async () => {
    const { clientB, roomCode } = await setupRoom();
    seedFreeKickSetup(roomCode); // away is kicking; default squad layout has no away piece on freeKickHex
  ...
  it('both teams ready (with valid placement) starts the kick: PASS, kicker as carrier, FREE_KICK_RESTART', async () => {
    ...
    // Wait for BOTH clients' copies of the home-ready broadcast before registering the
    // next listener, or clientB's listener for the second (both-ready) broadcast can
    // race against a still-in-flight copy of the FIRST broadcast.
